...
const CardBoard = ({ cardBoard, addCard, deleteCard, moveCard, cardBoardsDialog }) => {
  ...
  return (
    <div className={`cardboard ${cardBoard.id == 1 ? 'blue' : cardBoard.id == 2 ? 'purple' : cardBoard.id == 3 ? 'green' : ''}`}>
      ...
      {cardBoard.items.map((item, index) => <Card key={index} item={item} id={index} deleteCard={deleteCard} cardBoard={cardBoard} moveCard={moveCard} open={openDialog} dialogHandler={dialogHandler} cardBoardsDialog={cardBoardsDialog} />)}
